...
//  P6
//
//  Created by ellen chen on 2018/11/19.
...
const char *WINDOWTITLE = { "Firework" };
...
void Display() {
    ...
    for (int i = 0; i < 6; i++) {
           RotateY(&bar[i].p0, (bar[i].p0.y/HELIXHEIGHT)*180*sinf(1), 0, 0, 0);
          RotateY(&bar[i].p3, (bar[i].p0.y/HELIXHEIGHT)*180*sinf(1), 0, 0, 0);
...
void InitGraphics() {
    ...
    for (int i = 0; i < NUMBARS; ++i) {
        bar[i].color((float)i/NUMBARS,0. , 0.4+(float)i/NUMBARS);
        barheight = ( ((float)i / (NUMBARS-1)) * 2 * (HELIXHEIGHT-0.1));
